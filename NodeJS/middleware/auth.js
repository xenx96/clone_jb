...
import * as secretObj from "../config.js";

/**
 * For Login or Check, With JWT
 */
export const makeJWT = (formJson) => {
  return jwt.sign(formJson, secretObj.secret, { expiresIn: "30m" }); //formJSon Type is JSONObject.
};
...
export const loginValidate = async (AT) => {
  ...
  let timeComp =
    (await timeComparison(loginInfo.CA, loginInfo.LC)) >= COMP_FOR_TIME; //로그인 시간 비교
